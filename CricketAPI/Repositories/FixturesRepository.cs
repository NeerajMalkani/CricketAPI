...
namespace CricketAPI.Repositories
{
    public class FixturesRepository
    {
        ...
        public Fixtures_Balls? GetBallsWithOffset(DataContext context, long fixture_id, string innings_id, int start_index, int count)
        {
            Fixtures_Balls fixturesBalls = new Fixtures_Balls();
            try
            {
                List<BallJson> ballJson = context.BallJson.FromSqlRaw("CALL `cric_Get_FixtureBallsWithOffset`(" + fixture_id + ", '" + innings_id + "', " + start_index + ", " + count + ")").ToList();
                if (ballJson.Count > 0)
                {
                    fixturesBalls = JsonConvert.DeserializeObject<Fixtures_Balls>(ballJson[0].Fixtures_Balls) ?? throw new ArgumentException();
                }
